describe("Checkout Pages", () => {
  ...
  it("verifica o link de cancelamento", () => {
    cy.get(elements.CANCEL).click();
    cy.url().should("eq", "https://www.saucedemo.com/cart.html");
  });

  it("verificações tentando continuar com todos os campos em branco", () => {
  ...
  it("verifica a tentativa de continuar apenas com o primeiro nome", () => {
    cy.get(elements.ERROR).should("not.exist");
  ...
  it("verificações tentando continuar com todos os campos preenchidos", () => {
    cy.get(elements.ERROR).should("not.exist");
    cy.get(elements.FIRST_NAME).type("Test");
    cy.get(elements.LAST_NAME).type("Test");
  ...
  it("verifica o conteúdo na etapa dois", () => {
    cy.get(elements.FIRST_NAME).type("Test");
    cy.get(elements.LAST_NAME).type("Test");
    cy.get(elements.POSTAL_CODE).type("CV37 7JD");
    cy.get(elements.CONTINUE).click();
  ...
  it("verifica link de finalização", () => {
    cy.get(elements.FIRST_NAME).type("Test");
    cy.get(elements.LAST_NAME).type("Test");
    cy.get(elements.POSTAL_CODE).type("CV37 7JD");
    cy.get(elements.CONTINUE).click();
